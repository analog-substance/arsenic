...
func (svc *service) save(baseDir string) {
	...
	if svc.diffs.Length() > 0 {
		...
		if err != nil {
			logger.Error("failed to write resolv with diff file", "err", err)
			os.Exit(2)
		}
	}
	...
	if err != nil {
		logger.Error("failed to write other hostnames file", "err", err)
		os.Exit(2)
	}
	err = fileutil.WriteLines(filepath.Join(reconDir, "ip-addresses.txt"), svc.ipAddresses.SortedStringSlice())
	if err != nil {
		logger.Error("failed to write ip addresses file", "err", err)
		os.Exit(2)
	}
}
